    ...
    try:
        if user.count_documents({"email": email}) == 0:
            ...
            user_id = user.insert_one(
                {"name": name, "email": email, "password": hashed_password, "contact": contact, "token": ""}).inserted_id
...
def login_user(email: str, password: str, user, bcrypt) -> str:
    ...
    try:
        result = user.find_one(
            {"email": email}, {"password": password, "_id": 1})
        if compare_password(bcrypt, result['password'], password):
            # converted id into string then passed to encode_jwt
            token = encode_jwt(str(result['_id']))
            user.update_one({"email": email}, {
                            '$set': {"token": token.decode('utf-8')}})
            # need to decode JWT from bytes to string
            return dumps({'msg': 'Login Success!', "token": token.decode('utf-8')}), 200
...
def change_password(token: str, password: str, new_password: str, user, bcrypt) -> str:
    try:
        user_data = user.find_one(
            {"token": token}, {"password": password, "_id": 0})
...
def edit_profile(token: str, name: str, contact: str, user):
    try:
        ...
        updated_data = user.find_one(
            {"token": token}, {"name": name, "contact": contact, "_id": 0})
...
def get_user_detail(token: str, user):
    try:
        data = user.find_one({"token": token}, {
                             "name": 1, "email": 1, "contact": 1, "_id": 0})
